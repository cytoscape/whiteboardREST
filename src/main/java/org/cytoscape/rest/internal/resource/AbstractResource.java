...
public abstract class AbstractResource {
  ...
  public <T> T getService(Class<? extends T> clazz, String filter) {
    return manager.getService(clazz, filter);
  }

  abstract public String getResourceURI();
  abstract public Logger getResourceLogger();
  ...
  protected final String getGraphObject(int serializationErrorCode, final CyNetwork network, final CyIdentifiable obj) {
	...
	protected final Collection<Long> getByQuery(int networkNotFoundErrorCode, int invalidObjectTypeErrorCode, 
			                                        int invalidParameterErrorCode, final Long id, final String objType, 
																							final String column, final String query) {
    ...
    CyNetworkViewWriterFactory cytoscapeJsWriterFactory = manager.getViewWriterFactory();
  ...
  protected final Set<CyNetwork> getNetworksByQuery(int invalidParameterErrorCode, final String query, 
			                                              final String column) throws WebApplicationException {
  ...
  protected final RuntimeException getCIWebApplicationException(int status, String resourceURI, int code, String message, Logger logger, Exception e) {
